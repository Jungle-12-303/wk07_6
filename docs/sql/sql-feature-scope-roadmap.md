...
- malloc 과제에서 배운 free list, split, coalescing 개념이
  page allocator, B+Tree split/merge에 직접 연결된다.

### 적합한 상황

- 저장 구조를 깊게 이해하며 구현하고 싶을 때
- malloc/free-list, page, B+Tree delete를 연결해서 설명하고 싶을 때
- 발표에서 구조적 완성도와 시스템 관점 연결을 강조하고 싶을 때

### 기본 추천 이유

이번 프로젝트가 중시하는
`메모리 친화성`, `삭제 경로`, `방어 코드`, `B+ 트리 이해`,
그리고 CSAPP 9장 + malloc 구현 경험과의 연결을
가장 잘 만족시키는 범위이기 때문이다.
