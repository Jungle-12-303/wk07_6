...
C로 minidb를 짜면서 가장 두려웠던 건 메모리 버그였습니다. 세그멘테이션 폴트는 차라리 고맙습니다. 즉시 크래시하고 위치가 찍히니까요. 진짜 무서운 건 조용히 깨진 상태로 계속 돌아가는 프로그램입니다. 몇 시간 뒤 전혀 다른 위치에서 엉뚱하게 죽거나, 결과만 슬쩍 틀리는.
...
두 가지를 확인했습니다. 스택 오버플로우를 잡는 유일한 옵션은 ASan이었고, 그냥 실행하면 버그가 전혀 안 보입니다. 이 "전혀 안 보이는" 성질이 실제 버그의 80%를 어렵게 만듭니다. 내가 안 본 자리에 조용히 스택을 망가뜨리고 있을 뿐.
...
힙의 OOB는 스택보다 약간 더 쉽게 잡힙니다. 특히 valgrind가 힙 경계를 알고 있어서 확실합니다. 다만 valgrind는 느립니다 (실측 20-40배 감속). 유닛 테스트에선 쓸만하지만 벤치마크 용도로는 무겁습니다.

ASan은 보통 2-3배 감속에 그쳐서, minidb의 테스트 스위트에는 ASan이 기본이 됐습니다.
...
minidb에서 initial prototype에 UAF가 있던 자리는 B+ tree split 중 부모 노드 프레임이었습니다. split 로직이 자식 프레임을 pin한 채 부모 프레임을 unpin했고, 그 사이 다른 코드가 부모를 eviction했습니다. 부모 포인터가 이미 free된 메모리를 가리키고 있었지만, 50%의 확률로 여전히 올바른 값이라 테스트가 통과했습니다. pin_count를 도입하며 이 버그가 구조적으로 사라졌습니다 (application/02에서 다룬 바).
...
예방법은 단순하다. free 후 즉시 `p = NULL`. free된 포인터를 두 번 free해도 `free(NULL)`은 안전한 no-op이므로 버그가 닿지 않는다.
...
초기화되지 않은 값은 실행마다 다른 결과를 만듭니다. 테스트가 100번 통과해도 101번째에 실패할 수 있습니다. 이 버그는 컴파일러의 경고(`-Wall -Wextra -Wuninitialized`)로 잡는 게 제일 정확하고 빠릅니다.
...
C를 쓰면서 배우는 기술 중 70%는 어떻게 쓰는가지만, 나머지 30%는 무엇이 잘못될 수 있는가에 대한 직관입니다. 이 직관은 책으로는 잘 안 쌓이고, 실제 크래시를 몇 번 겪으면서만 쌓입니다. 의도적으로 크래시를 만들어 보는 것은 이 직관을 압축된 시간에 주입하는 방법이었습니다.
