...
라인으로 메모리를 덮는 보통의 캐시와 달리, `TLB` 한 엔트리가 덮는 범위는 페이지 크기인 **4 KB**입니다.
...
**한 엔트리가 덮는 범위**를 늘려 같은 `TLB` 용량으로 더 많은 주소 공간을 커버합니다.
...
`TLB`는 메모리를 덮는 캐시가 아니라 **번역 결과를 덮는 캐시**입니다.
