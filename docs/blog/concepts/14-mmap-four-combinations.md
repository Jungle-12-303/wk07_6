...
`mmap`은 파일이나 익명 메모리를 프로세스의 가상 주소 공간에 매핑하는 시스템 콜이다.

`read`·`write`와 달리, 커널-유저 사이에 데이터 복사가 없다.
프로세스는 매핑된 주소를 보통의 메모리처럼 접근한다.

그런데 `mmap`의 의미는 두 축—**file-backed vs anonymous**와 **shared vs private**—의 네 가지 조합에서 결정된다.
이 조합이 정말 중요하다.
...
이 호출은 내부적으로 프로세스의 `mm_struct`에 새 VMA를 추가한다.
실제 페이지 테이블은 비어 있는 상태로 놓인다.
접근 시 `page fault`를 통해 페이지가 채워진다 — 지연 매핑이다.
...
**MAP_SHARED**:
- 매핑된 페이지의 쓰기가 다른 프로세스(같은 매핑을 공유하는)에게 즉시 보인다.
- 파일에도 동기화된다.

**MAP_PRIVATE**:
- 매핑된 페이지의 쓰기는 COW로 처리된다.
- 쓰기가 일어나는 순간 그 프로세스만의 사본이 만들어진다.
- 파일에는 반영되지 않는다.
...
프로그램이 `execve`로 시작되면 커널은 ELF의 text·rodata·data 세그먼트를 **file-backed + private**으로 매핑한다.

읽기는 파일에서 바로 올라온다 (major fault).
쓰기가 가능한 data 영역은 처음 쓸 때 COW로 분리되어 파일 원본은 변하지 않는다.

흥미로운 부분은 같은 실행 파일을 여러 번 실행할 때다.
text 프레임들이 **물리적으로 공유**되므로 메모리 사용량이 훨씬 줄어든다.
...
`write`·`read`를 계속 부르지 말고, **파일을 메모리처럼 다룬다**.

같은 파일을 `MAP_SHARED`로 매핑한 다른 프로세스는 같은 물리 프레임을 본다.
한쪽의 쓰기가 즉시 보인다.
이 성질이 데이터베이스 버퍼 풀, 로그 파일 공유 등에서 활용된다.

### Anonymous + Private — malloc의 일부

glibc의 `malloc`은 작은 요청에는 `brk`를 쓴다.
그런데 큰 요청(기본적으로 128 KB 이상)에는 **anonymous + private** `mmap`을 쓴다.
...
이 매핑은 호출 즉시 0으로 채워진 가상 주소 공간을 제공한다.
`free` 시점에 `munmap`으로 돌려줄 수 있다.

그래서 큰 할당을 해제하면 그 주소 공간이 **완전히 OS로 반환**된다.
`brk`로 받은 힙 메모리는 반환이 쉽지 않은 것과 대비된다.

### Anonymous + Shared — fork 후 공유 메모리

부모가 `fork` 전에 anonymous+shared로 매핑을 만들면, 그 영역은 자식에서도 **같은 물리 프레임**을 가리킨다.
COW가 적용되지 않으므로 쓰기가 즉시 공유된다.
프로세스 간 공유 변수, 세마포어 등에 쓸 수 있다.
...
`mmap`은 "파일/익명"과 "공유/사적"의 두 축을 교차시켜 네 가지 의미를 만든다.

실행 파일 로딩도, `malloc`의 일부도, 프로세스 간 공유 메모리도, 데이터베이스 파일 매핑도 모두 이 네 조합 중 하나다.

시스템 콜 하나가 이렇게 넓은 스펙트럼의 작업을 감당할 수 있는 이유는, 네 조합 모두 결국 **VMA 등록 + 지연 매핑 + 적절한 권한**이라는 공통 기반 위에 서 있기 때문이다.
