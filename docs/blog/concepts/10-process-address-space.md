...
컴파일러·링커·로더·커널이 합의한 **정해진 레이아웃**을 따릅니다.
...
이 영역이 읽기 전용이라는 사실이 **같은 바이너리를 여러 프로세스가 공유하는 것**을 가능하게 합니다.
...
리눅스는 프로세스 시작 시 스택에 **환경 변수**와 **`argv`** 를 미리 배치해 둡니다.
...
사실 커널은 언제나 주소 공간에 걸려 있지만 **권한 검사**에 의해 유저에게 보이지 않을 뿐입니다.
...
- **`null page`**: 주소 0번지 근처에는 어떤 매핑도 두지 않습니다.
  ...
  값이 0인 포인터를 역참조하면 반드시 `fault`가 나도록 보장합니다.
...
흥미로운 건, 프로그램을 실행하며 각 변수의 주소를 출력해 보면 레이아웃이 눈에 들어온다는 것입니다.
...
프로세스 주소 공간은 자유롭게 쓸 수 있는 거대한 바이트 배열이 아니라, **각 영역이 고유한 권한과 용도를 가진 구획**입니다.
