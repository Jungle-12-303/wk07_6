# 다단계 페이지 테이블: 거대한 공간을 현실적으로 관리하다
...
대부분의 가상 주소 공간은 **비어 있다**는 사실을 활용하지 못합니다.
...
각 레벨의 테이블은 **4 KB 한 페이지**로 구성되며, 512개의 8바이트 엔트리를 담습니다. 그래서 각 단계의 인덱스가 9비트(2^9 = 512)입니다. 번역은 CPU의 **CR3 레지스터**에 담긴 PML4 테이블의 물리 주소에서 시작합니다.
...
3. 각 레벨에서 엔트리의 **Present(P) 비트**가 1이고, 요구한 접근 종류가 해당 엔트리의 권한(W, U, NX)에 맞는지 확인합니다.
4. 어느 레벨에서든 조건이 어긋나면 CPU에 **`page fault`** 또는 **일반 보호 예외**를 던집니다.
5. 조건이 모두 맞으면 최종 PFN과 12비트 오프셋을 이어 붙여 **물리 주소**를 만들어 메모리 버스에 탑니다.
...
커널은 테이블의 내용을 채워 넣을 뿐이고, **걸어 내려가는 일은 전적으로 하드웨어의 몫**입니다.
...
- **2 MB page**: PD 레벨의 엔트리가 하위 PT를 가리키는 대신 **바로 2 MB 프레임**을 가리킵니다.
...
하드웨어가 번역을 수행한다고 해도, **테이블의 내용물**을 채우는 일은 언제나 커널의 책임입니다.
