...
C++, Java, Python에서는 **클래스**로 데이터와 동작을 묶는다.
C에는 클래스가 없다. 대신 **바이트 배열(`uint8_t*`) + packed 구조체 + memcpy**로
동일한 추상화를 달성한다.

minidb에서 이 패턴이 프로젝트 전체를 관통하는 핵심 설계이다.
...
**직렬화/역직렬화가 없다.** 메모리의 바이트 배치가 곧 디스크 포맷이기 때문이다.

---

## 3. `__attribute__((packed))` — 패딩 제거

C 컴파일러는 기본적으로 구조체 필드 사이에 **패딩**을 넣어 정렬한다.
...
```
디스크에 기록할 때 구조체를 memcpy로 통째로 쓴다.
패딩이 있으면 디스크에 쓸모없는 바이트가 끼어들어
다른 플랫폼에서 읽을 때 필드 위치가 어긋난다.
...
minidb의 가장 인상적인 패턴은 **같은 4096바이트를 page_type에 따라 다른 구조체로 읽는 것**이다.
...
이것은 C++의 **다형성(polymorphism)**을 메모리 레벨에서 구현한 것이다.
vtable이나 상속 체인 없이, 첫 4바이트(page_type)가 타입 판별자 역할을 한다.
...
minidb는 구조체를 읽을 때 항상 `memcpy`를 사용한다:
...
헤더 뒤의 엔트리 배열에 접근할 때 포인터 산술을 사용한다:
...
메모리 할당 없이 페이지 버퍼 위에서 직접 동작한다.

---

## 8. 슬롯 기반 간접 참조 — row_ref_t

힙에서 행의 위치를 나타내는 `row_ref_t`도 같은 패턴이다:
...
이것은 **2단계 간접 참조(double indirection)**이다:
...
이 패턴의 핵심은 **"메모리와 디스크 사이의 경계를 없앤다"**는 것이다.
packed 구조체 덕분에 memcpy 한 번으로 디스크와 메모리를 오갈 수 있고,
page_type이라는 4바이트 태그로 런타임 다형성을 구현한다.
클래스 계층 구조와 직렬화 코드 없이도 동일한 추상화를 달성하며,
오히려 메모리 효율과 디스크 I/O 성능에서 더 유리하다.
