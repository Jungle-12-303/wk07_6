 ...
 *
 * 주요 연산:
 *   - 검색: root에서 leaf까지 internal_find()로 자식을 선택하며 내려감. O(log n)
...
/*
 * internal_child_for_key - internal 노드에서 key가 속할 자식 페이지 ID를 반환한다.
 *
...
/*
 * leaf_insert_entry - leaf 페이지에 (key, row_ref) 엔트리를 정렬 유지하며 삽입한다.
 *
 * 동작:
...
static void split_leaf(pager_t *pager, uint32_t leaf_pid, uint64_t key, row_ref_t ref)
{
    ...
    leaf_page_header_t new_lph = {
        ...
        .next_leaf_page_id = lph.next_leaf_page_id,  /* 기존 다음 이웃을 물려받음 */
...
static void fix_leaf_after_delete(pager_t *pager, uint32_t leaf_pid)
{
     ...
     *
     * internal 노드의 자식 구조:
     *   leftmost_child(child_idx=0) | key[0] | child[0](child_idx=1) | key[1] | child[1](child_idx=2) | ...
     *
